    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                .player-avatar { width: 28px; height: 28px; border-radius: 4px; background: #1f2937; }
            ...
            <script>
                ...
                async function loadLeaderboard(category = 'overall') {
                    ...
                    try {
                        ...
                        // توحيد الحقول لتتوافق مع أسماء الأعمدة في قاعدة البيانات (ign, username, tier, rank_earned)
                        const processedData = data.map(row => {
                            const playerName = row.ign || row.username || row.player || 'Unknown';
                            const playerTier = row.tier || row.rank_earned || 'N/A';
                            const playerGamemode = (row.gamemode || category).toUpperCase();
                            const points = row.points || (typeof playerTier === 'string' && playerTier.startsWith('HT') ? 150 : 100);
                        ...
                        tbody.innerHTML = processedData.map((row, index) => {
                            // جلب سكن ماينكرفت الأصلي، وإذا لم يوجد أو كان كراكد يتحول تلقائياً إلى سكن ستيف الافتراضي
                            const skinUrl = \`https://minotar.net/avatar/\${row.playerName}/28.png\`;
                            const steveUrl = "https://minotar.net/avatar/MHF_Steve/28.png";
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="player-cell">
                                            ...
                                            <span style="font-weight: bold; color: #ffffff;">\${row.playerName}</span>
                                        </div>
                                    </td>
                                    <td style="text-transform: uppercase;">\${row.playerGamemode}</td>
                                    <td><span style="color: #facc15; font-weight: bold;">\${row.playerTier}</span></td>
                                    <td>\${row.points}</td>
                                </tr>
                            \`;
                        }).join('');
